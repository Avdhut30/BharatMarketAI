...
def mf_dashboard():
    ...
    if not uploaded:
        st.info("Upload your CSV to view the dashboard.")
        return
    ...
    # ----------------------------
    # ✅ Use NAV HISTORY (not latest only)
    # ----------------------------
    from src.mf.data.nav_history import get_nav_history_for_isins
    nav_df, meta = get_nav_history_for_isins(isins, force_refresh=False, max_years=10)
    ...
    # Ensure schema expected by metrics.py
    ...
    # Re-apply filters after live edits
    filtered = summary.copy()
    if q.strip():
        filtered = filtered[filtered["scheme_name"].astype(str).str.contains(q.strip(), case=False, na=False)]
    if hide_zero and "units" in filtered.columns:
    ...
    with st.expander("💰 Realized vs Unrealized (FIFO)"):
        if not is_txn:
            ...
            # FIFO needs 'units' in transactions (your compute_realized_unrealized_fifo typically expects it)
            if "units" not in txn_df.columns:
                st.warning("Your transactions CSV does not include `units` column. FIFO realized/unrealized needs units.")
            else:
                try:
                    ...
                    if ru is None or (isinstance(ru, pd.DataFrame) and ru.empty):
                        ...
                        if isinstance(ru, pd.DataFrame):
                            if "realized_gain" in ru.columns:
                                st.metric("Total Realized Gain", _currency(pd.to_numeric(ru["realized_gain"], errors="coerce").fillna(0).sum()))
                            if "unrealized_gain" in ru.columns:
                                st.metric("Total Unrealized Gain", _currency(pd.to_numeric(ru["unrealized_gain"], errors="coerce").fillna(0).sum()))
    ...
    # Build table rows for PDF (keep short + readable)
