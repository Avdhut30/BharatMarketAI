...
def _flatten_yf_columns(df: pd.DataFrame, symbol: str) -> pd.DataFrame:
    ...
    if isinstance(df.columns, pd.MultiIndex):
        # Prefer slice for requested symbol
...
def load_news_daily() -> pd.DataFrame | None:
    ...
    try:
        ...
        # keep only numeric columns (news features)
...
def merge_news_features(feat: pd.DataFrame) -> pd.DataFrame:
    ...
    This makes Advisor compatible with models trained with news features.
    """
    nd = load_news_daily()
...
def score_single_symbol(
    ...
    # If feature cols missing in bundle, infer (not recommended)
