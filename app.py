...
if page == "🏠 Overview":
    ...
    if BEST_CONFIG:
        st.success(f"Auto-loaded best config → {BEST_CONFIG}")
    else:
    ...
    with colX:
        run_best = st.button("🏆 Run Best Config")
    with colY:
    ...
    st.subheader("🧠 Single Stock / Fund Advisor (Buy / Hold / Sell)")
    ...
    if run_one:
        try:
            ...
            st.subheader("📰 News Features (used by model)")
